...
function Reflection(){ //Функция для определения свойства Рефлексивность
    ...
    for(let i = 0; i < mainMass.length; i++){ // цикл по всей строке пар элементов
        ...
        for(let j = 0; j < mass1.length; j++){ // обнуление флага при переходе к след паре элементов
            flag = 0;
                for(let k = 0; k < rMass.length; k++){ //Проверка элементов из пар между массивами
                    if(mass1[j] == rMass[k]) 
                    {
                        flag ++;
                    }
...
function Transition(){ //функция на определение транзитивности
    ...
    for(let i = 0; i < mainMass.length; i++){
        flag = 0;
        mass1 = mainMass[i].split(" "); // получает 1 пару элементов
        for(let j = 1; j < mainMass.length; j++ ){
            mass2 = mainMass[j].split(" "); //получает  2 пару элементов
            for(let k = 2; k < mainMass.length; k++){
                mass3 = mainMass[k].split(" "); //получает 3 пару элементов

		...
		<div class="cent">
			<form method="post">
				...
				<textarea id="matr" class="matrica" name = 'matrica' value = '0'></textarea><br>
					...
					<input type = 'number' class="spin" min = '1' placeholder="1" name = 'start' value = '<?= $_POST[start]?>'>
					вершины в 
					<input type = 'number' class="spin" min = '1' placeholder="1" name = 'end' value = '<?= $_POST[end]?>'>
			...
			<?php
				...
				for($i = 0; $i < count($matrica); $i++) {
					...
					if (count($matrica[0]) != count($matrica[$i]) or count($matrica) != count($matrica[$i])) { //Проверка на ввод матрицы
				...
				echo '<br>';
				echo 'Путь из ' . ($start + 1) . ' вершины в ' . $_POST[end] . ' вершину составляет ';
				echo '(';
